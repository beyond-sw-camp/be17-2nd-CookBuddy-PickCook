...
const submitReply = async () => {
  ...
  if (replyImage.value) {
    try {
      ...
      formData.append('file', replyImage.value)
      const presignedData = await api.getPresignedUrl(formData)

      if (presignedData.success) {
        const presignedUrl = presignedData.results

        // 2️⃣ S3 업로드
        await api.uploadImage(presignedUrl, replyImage.value)

        // 3️⃣ 화면/서버에 저장될 URL
        imageUrl = presignedUrl.split('?')[0]
      }
    } catch (err) {
      console.error('대댓글 이미지 업로드 실패:', err)
    }
  }
