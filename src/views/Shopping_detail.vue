...
onMounted(async () => {
    try {
        ...
        // 리뷰 데이터 가져오기
        const reviewData = await reviewAPI.getProductReviews(productId.value)
        reviews.value = reviewData.reviews
        console.log('받은 리뷰 데이터:', reviews.value)
    } catch (error) {
...
<div v-else-if="product" class="shopping-detail-container">
    ...
    <div class="product-info-section">
        ...
        <div v-if="reviews && reviews.length > 0" class="reviews">
            <h4>고객 리뷰</h4>
            <div v-for="review in reviews" :key="review.userId" class="review-item">
